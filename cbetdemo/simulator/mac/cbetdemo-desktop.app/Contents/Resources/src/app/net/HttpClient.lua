...
function HttpClient:start()
	...
	local function onRequestFinished(event)
			...
			if code==200 then
				...
				--获取网络响应头信息
	  			local respHeadStr = request:getResponseHeadersString()
	  			print("响应头参数:",respHeadStr)
				...
				dump(responseData,"httpclient responseData",6)
			    ...
			    --网络通讯异常状态码  非200 回调	
				self:netErrorHandler(code,request:getErrorMessage())
			end
	end
	--END 网络请求响应函数 
	...
	if self.needEntryData then
		...
		if device.platform =="android" then
			self.netRequest:addRequestHeader("Content-Type=application/ews_"..DataTokenInstance:getInstance():getType())
		else
			self.netRequest:addRequestHeader("Content-Type:application/ews_"..DataTokenInstance:getInstance():getType())
		end
		...
		--android 平台请求头 使用 = 连接
		if device.platform =="android" then
			self.netRequest:addRequestHeader("Content-Type=application/json")
		else
			self.netRequest:addRequestHeader("Content-Type:application/json")
		end
	...
	self.netRequest:setPOSTData(entryData)
	self.netRequest:start()
